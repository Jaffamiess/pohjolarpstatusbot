...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("PohjolaRP STATUS", "https://cdn.discordapp.com/attachments/688387033927778434/700753668986634250/Uusilogo.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪PohjolaRP' : '⚫ PohjolaRP')
    .setTimestamp(new Date())
    //.addField('Tervetuloa pelaamaan!',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('Palvelimen tila',':x: **EI PÄÄLLÄ** ',true)
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Palvelimen tila',':white_check_mark: **PÄÄLLÄ**',true)
        //.addField('Wachtrij',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Pelaajat',`**${players.length}/${MAX_PLAYERS}\n\u200b\n**`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
          ...
          for (var i=0;i<fields.length;i++) {
            let field = fields[i];
            if (field.length > 0) embed.addField('ydr\u200bggg',field,true);
          }
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('PohjolaRP',{'url':'https://www.twitch.tv/jaffamiess','type':'STREAMING'});
